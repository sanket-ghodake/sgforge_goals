  ...
  return `
    window.openReviewDrawer = function(boardId) {
      ...
      fetch('api/boards/' + boardId)
        ...
        .catch(function(err) {
          drawerPane.innerHTML = '<div style="padding:40px; text-align:center; color:var(--forge-error);">' + err.message + '</div>';
        });
    };
    ...
    function renderDrawerContent(board) {
      ...
      chatHtml += \`<div class="chat-bubble chat-bubble-system">Goal Board Created for \${board.projectName || 'Project'} (\${board.cycle})</div>\`;
        ...
        chatHtml += \`
          <div class="chat-bubble chat-bubble-employee">
            <div class="chat-author">
              <span>\${board.ownerName} (\${board.ownerDepartment})</span>
            ...
            <div style="font-size: 0.8rem; color: var(--forge-text-muted);">
              Revision \${board.revisionNumber} submitted with \${itemLen} committed milestones.
            </div>
          </div>
        \`;
      }
          ...
          if (isReworkMsg) {
            chatHtml += \`
              <div class="chat-bubble chat-bubble-rework">
                <div class="chat-author" style="color: var(--forge-warning);">
                  ...
                  <span class="chat-timestamp">\${msgTime}</span>
                </div>
            ...
            chatHtml += \`
              <div class="chat-bubble chat-bubble-approved">
                ...
                <div style="font-size: 0.85rem; color: var(--forge-text-main); margin-bottom: 6px;">\${c.commentText}</div>
                <div class="chat-timestamp">Approved by \${c.authorName} &bull; \${msgDate}</div>
              </div>
            \`;
            ...
            chatHtml += \`
              <div class="chat-bubble chat-bubble-system">
                Unlocked by \${c.authorName} &bull; \${msgTime}
              </div>
            \`;
            ...
            const itemBadgeHtml = matchedItem ? '<div style="margin-bottom: 4px;"><span style="font-size:0.7rem; font-weight:700; background:rgba(59,130,246,0.15); color:#60a5fa; border:1px solid rgba(59,130,246,0.3); border-radius:4px; padding:2px 6px;">' + matchedItem.title + '</span></div>' : '';
            chatHtml += \`
      ...
      if (isApproved && (!board.comments || !board.comments.some(function(c) { return c.type === 'APPROVAL_NOTE'; }))) {
        chatHtml += \`
          <div class="chat-bubble chat-bubble-approved">
            ...
            <div style="font-size: 0.825rem; color: var(--forge-text-muted);">Signed by \${board.approvedBy || 'Manager'}</div>
          </div>
        \`;
      }
      ...
      const safeTitle = (board.title || '').replace(/'/g, "\\\\'");
      ...
      const actionButtons = \`
        \${isSubmitted || isRework || isOverdue || isUnlockReq ? \`
          <button class="btn-action btn-outline" style="font-size: 0.75rem; color: var(--forge-warning); border-color: rgba(245, 158, 11, 0.3);" onclick="openReworkModal('\${board.id}', '\${safeTitle}')">
            ${icons.alertCircle} Request Rework
          </button>
          <button class="btn-action btn-primary" style="font-size: 0.75rem;" onclick="handleApproveBoard('\${board.id}')">
        ...
        \` : isApproved ? \`
          ...
          <button class="btn-action btn-outline" style="font-size: 0.75rem;" onclick="handleUnlockBoard('\${board.id}')">
      ...
      drawerPane.innerHTML = \`
          ...
          <div>
            <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 2px;">
              <span style="font-size: 0.75rem; font-weight: 700; color: var(--forge-primary);">
                \${board.projectName || 'Project'} &bull; \${board.cycle}
              </span>
            ...
            <h3 style="font-size: 1.1rem; font-weight: 700; margin: 0 0 2px 0;">\${board.title}</h3>
            <div style="font-size: 0.775rem; color: var(--forge-text-muted);">
              Contributor: <strong style="color: var(--forge-text-main);">\${board.ownerName}</strong> &bull; Rev \${board.revisionNumber}
            </div>
          </div>
        ...
        <div class="drawer-footer">
          <div style="display: flex; gap: 8px; margin-bottom: 10px;">
            ...
            <input id="drawerCommentInput" type="text" placeholder="Write feedback or critique..." style="flex: 1; height: 38px; border-radius: 10px; background: var(--forge-bg-card); border: 1px solid var(--forge-border); color: var(--forge-text-main); padding: 0 14px; font-size: 0.85rem;" onkeydown="if(event.key==='Enter') submitDrawerComment('\${board.id}')" />
            <button class="btn-action btn-primary" style="height: 38px;" onclick="submitDrawerComment('\${board.id}')">
              ${icons.send} Send
            </button>
          </div>
          ...
          <div style="display: flex; justify-content: space-between; align-items: center; padding-top: 10px; border-top: 1px solid var(--forge-border);">
            <a href="?tab=board&id=\${board.id}" onclick="closeReviewDrawer(); navigateSpa('board', '\${board.id}', event)" class="btn-action btn-outline" style="font-size: 0.775rem;">
