...
import type { AuthUser, CreateBoardInput, GoalBoard, GoalItem, Project, UpdateGoalItemsInput } from '../../lib/types';
...
export function listProjects(orgId: string): Project[] {
  return goalsDb.query<any, [string, string]>('SELECT * FROM projects WHERE org_id = ? OR org_id = ? ORDER BY name ASC').all(orgId, 'org_default').map(p => ({
    id: p.id,
    orgId: p.org_id,
    name: p.name,
    code: p.code,
...
export function listBoards(orgId: string, filter?: { ownerId?: string; projectId?: string; cycle?: string }): GoalBoard[] {
  ...
  const rows = goalsDb.query<any, any[]>(query).all(...params);
  return rows.map(r => ({
    id: r.id,
    orgId: r.org_id,
...
export function getBoardById(boardId: string, orgId: string): GoalBoard {
  ...
  const comments = goalsDb.query<any, [string]>(`
...
export function assertBoardMutable(board: GoalBoard, user: AuthUser): void {
  ...
  if (board.status === 'APPROVED') {
    throw new BoardLockedError('This board has been approved and permanently sealed. Modifications are locked.');
  }
}
...
export function submitBoard(boardId: string, user: AuthUser): GoalBoard {
  ...
  goalsDb.transaction(() => {
    ...
    goalsDb.run(`
      ...
      VALUES (?, ?, 'usr_manager', ?, 'PENDING_APPROVAL', ?, null, 0, ?)
    `, [
      remId,
      orgId,
      boardId,
    ...
    // Dismiss existing submission/rework reminders for this board
    goalsDb.run(`
