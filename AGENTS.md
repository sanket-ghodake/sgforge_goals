...
4. [ ] **Modern Portable Design System (shadcn + Magic UI + Aceternity + Luxe)**:
   ...
   - **Zero Browser/OS Defaults Invariant (Hard Blocked in Pre-Commit Gate)**:
     ...
     - **Dialogs & Popups**: Raw browser `alert()`, `confirm()`, and `prompt()` are strictly forbidden. Destructive or irreversible actions (such as permanent goal plan signoff or milestone deletion) must use the universal glassmorphic confirmation dialog (`window.showModernConfirm`).
