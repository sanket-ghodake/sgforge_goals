...
4. [ ] **Modern Portable Design System (shadcn + Magic UI + Aceternity + Luxe)**:
   ...
   - **Zero Browser/OS Defaults Invariant (Hard Blocked in Pre-Commit Gate)**:
     - **Dropdowns**: Raw native OS `<select>` tags are strictly forbidden. All selectors must use custom glassmorphic dropdowns (`renderModernSelectHtml` / `.modern-select`) with smooth popover physics, custom SVG chevron arrows, and keyboard accessibility.
     - **Sliders**: Raw native `<input type="range">` elements are strictly forbidden. All sliders must use modern styled sliders (`.modern-slider`) with dynamic gradient progress fill tracks, tactile glassmorphic thumbs, and real-time value badges.
     - **Dialogs & Popups**: Raw browser `alert()`, `confirm()`, and `prompt()` are strictly forbidden. Destructive or irreversible actions (such as permanent flight plan signoff or milestone deletion) must use the universal glassmorphic confirmation dialog (`window.showModernConfirm`).
     - **Tooltips**: Browser default tooltips via `title="..."` attributes on interactive elements are strictly forbidden. Must strictly use `data-astryx-tooltip="..."` and the custom animated `#astryx-tooltip` engine with collision detection.
     - **Notifications**: All toasts must use the modern glassmorphic toast engine (`modernToast` / `astryxToast`) featuring animated countdown progress bars, manual dismiss close buttons, and type-based accent borders.
